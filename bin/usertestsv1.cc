...
writebig(char *s)
{
  ...
  for(i = 0; i < MAXFILE; i++){
    ...
    if(write(fd, buf, BSIZE) != BSIZE){
      printf("%s: error: write big file failed %d\n", s, i);
      exit(1);
    }
  }
...
pgbug(char *s)
{
  char *argv[1];
  argv[0] = 0;
  execv((char*)0xeaeb0b5b00002f5e, argv);
...
countfree()
{
  ...
  while(1){
    if((uint64_t)sbrk(4096) == 0xffffffffffffffff){
      break;
    }
    n += 1;
  }
  sbrk(-((uint64_t)sbrk(0) - sz0));
  return n;
}
